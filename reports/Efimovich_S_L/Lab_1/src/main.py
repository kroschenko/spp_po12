class LimitedSet:
    ...
    def union(self, other):
        if not isinstance(other, LimitedSet): raise TypeError("Ожидается LimitedSet")
    ...
    def __str__(self):
        return f"LimitedSet({{{', '.join(map(str, self._items))}}}, {self.size}/{self._cap})" if self._items else f"LimitedSet(пусто, {self._cap})"
