...
def build_map():
    ...
    all_projects = []
    if repos: all_projects.extend(repos)
    if starred: all_projects.extend(starred)
    ...
    for repo in all_projects[:10]:
        ...
        contributors = get_github_data(f"https://api.github.com/repos/{repo_name}/contributors")
        if contributors:
    ...
    # Рисуем тебя
    nx.draw_networkx_nodes(G, pos, nodelist=[USERNAME], node_color='orange', node_size=1000)
    # Рисуем остальных
