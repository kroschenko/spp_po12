...
@app.post("/components/")
def create_component(name: str, price: float, cat_id: int, man_id: int, s: Session = Depends(get_db)):
    new_item = db.Component(name=name, price=price, category_id=cat_id, manufacturer_id=man_id)
    s.add(new_item)
    s.commit()
    s.refresh(new_item)
    return new_item
