# pylint: disable=invalid-name, too-many-instance-attributes, broad-exception-caught, too-few-public-methods
"""
...
class Line:
    ...
    def get_side(self, p: Point) -> float:
        ...
        return (p.x - self.x1) * (self.y2 - self.y1) - (p.y - self.y1) * (self.x2 - self.x1)
    ...
    def update_animation(self):
        ...
        if not self.is_paused:
            ...
            self.canvas.create_line(
                self.line.x1, self.line.y1, self.line.x2, self.line.y2, fill="black", width=3
            )
