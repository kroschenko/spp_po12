...
def save_visualizations(topic: str, repositories: list[RepositoryInfo]) -> Path:
    ...
    most_popular = data.nlargest(TOP_REPOSITORIES, "stars").sort_values("stars")
    ...
    axes[0].set_title("Популярные языки программирования")
    axes[0].set_xlabel("Количество репозиториев")
    ...
    axes[1].set_title("Популярность и активность репозиториев")
    axes[1].set_xlabel("Количество звёзд")
    axes[1].set_ylabel("Открытые issues")
    color_bar = figure.colorbar(scatter, ax=axes[1])
    ...
    axes[2].set_title("Топ репозиториев по звёздам")
    axes[2].set_xlabel("Количество звёзд")
    axes[2].set_ylabel("Репозиторий")
    ...
    inset.set_title("Старение репозиториев", fontsize=10)
    inset.set_xlabel("Дней без обновления", fontsize=9)
    inset.set_ylabel("Частота", fontsize=9)
